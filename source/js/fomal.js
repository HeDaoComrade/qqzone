...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | 子非晚香")[0];
}
...
$.ajax({
  ...
  data: {
    key: 'EPPBZ-KL2LB-I6WUO-NBL4E-I57XK-WKFKW',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  let dist = getDistance(107.4137911, 29.7062570, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
  ...
  switch (ipLoacation.result.ad_info.nation) {
    case "日本":
      posdesc = "よろしく，but you are not welcome.";
      break;
    case "美国":
    ...
    case "中国":
      ...
      switch (ipLoacation.result.ad_info.province) {
        case "北京市":
          ...
          break;
        case "重庆市":
          posdesc = "哇塞，你和站长是在同一个城市耶！哪天带你去吃火锅（bushi";
          break;
        case "天津市":
          posdesc = "讲段相声吧。";
          break;
  ...
  else if (date.getHours() >= 15 && date.getHours() < 16) timeChange = "<span>三点几啦</span>，一起喝杯咖啡呀！";
  ...
  try {
    ...
    document.getElementById("welcome-info").innerHTML =
      `<b><center>🎉 欢迎信息 🎉</center>&emsp;&emsp;诶嘿！欢迎来自 <span style="color:var(--theme-color)">${pos}</span> 的小伙伴，${timeChange}你现在距离站长在物理上约有 <span style="color:var(--theme-color)">${dist}</span> 公里，当前的IP地址为： <span style="color:var(--theme-color)">${ip}</span>， ${posdesc}</b>`;
  } catch (err) {
...
function showWelcome() {
  try {
    ...
    console.log("Pjax无法获取#welcome-info元素🙄🙄🙄", err);
  }
}
...
function setDarkModeClass(t) {
  ...
  if (e) {
    ...
    console.warn("darkmode element not found, skip setAttribute");
  }
}
...
function initSnow(canvasId, flakeCount) {
  const i = document.getElementById(canvasId);
  if (!i) {
    console.warn("Canvas element not found for snow");
    return;
  }
...
// ================== Example: Encryption Stub ==================
function encryptPost(content, password) {
  if (!password) return content;
  try {
    return "encrypted:" + content; // 占位逻辑
  } catch (err) {
    console.error("encryptPost error:", err);
    return content;
  }
}
...
if ((navigator.userAgent.match(/(Symbian|Windows Phone)/i))) {
  ...
  window && (() => {
    ...
    if (!i) {
      console.warn("未找到 #snow canvas，跳过初始化");
      return; // 若在函数中
     // 或者写成：
      // throw new Error("canvas #snow 不存在");
    }
    const n = i.getContext("2d");
    const o = e.flakeCount;
    let a = -100,
        d = -100,
        s = [];
...
function createtime2() {
  ...
  setTimeout(console.log.bind(console, "%c WELCOME %c 欢迎光临合道同志的小站！", "color:white; background-color:#23c682", ""));
